...
export function DebtDetail({id}:{id:number}){
 const [data,setData]=useState<Detail|null>(null),[loading,setLoading]=useState(true),[error,setError]=useState(""),[payOpen,setPayOpen]=useState(false),[amount,setAmount]=useState(0),[method,setMethod]=useState<Method>("CASH"),[notes,setNotes]=useState(""),[busy,setBusy]=useState(false),[message,setMessage]=useState(""),[today]=useState(todayInTashkent);
 ...
 return <div className="debt-page debt-detail-page">
  ...
  <section className="debt-balance"><div><span>Qolgan qarz</span><strong>{formatMoney(data.remainingAmount)}</strong></div><em className={data.status==="PAID"?"paid":"late"}>{data.status==="PAID"?"To‘langan":data.dueDate<today?"Muddati o‘tgan":"Joriy"}</em><dl><dt>Jami qarz</dt><dd>{formatMoney(data.totalAmount)}</dd><dt>To‘langan</dt><dd>{formatMoney(data.paidAmount)}</dd><dt>To‘lov muddati</dt><dd>{data.dueDate}</dd><dt>Sotuv raqami</dt><dd>{data.saleNumber}</dd></dl>{data.status==="ACTIVE"&&<button onClick={()=>{setAmount(data.remainingAmount);setPayOpen(true)}} className="debt-pay-button">＋ To‘lov qo‘shish</button>}</section>
  <section className="debt-product">{data.imageUrl?<OptimizedImage src={data.imageUrl} alt={`${data.brand} ${data.model}`} width={96} height={112} sizes="96px"/>:<div className="debt-avatar"><User/></div>}<div><small>Qarzga olingan telefon</small><strong>{data.brand} {data.model}</strong><span>{[data.storage,data.color,data.platform].filter(Boolean).join(" • ")}</span></div></section>
