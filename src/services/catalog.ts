...
import { media, products, purchaseBatches } from "@/db/schema";
...
export async function listInventoryPage(search = "", category?: "SMARTPHONE" | "FEATURE_PHONE",options:InventoryOptions={}) {
  const db = getDb();
  const primaryMedia=db.select({
    productId:media.productId,
    imageUrl:sql<string|null>`(array_agg(${media.url} order by ${media.sortOrder},${media.id}))[1]`.as("image_url")
  }).from(media).groupBy(media.productId).as("primary_media");
...
export async function listPublicProducts(category?: "SMARTPHONE" | "FEATURE_PHONE") {
  ...
  const db = getDb();
  const primaryMedia=db.select({productId:media.productId,imageUrl:sql<string|null>`(array_agg(${media.url} order by ${media.sortOrder},${media.id}))[1]`.as("image_url")}).from(media).groupBy(media.productId).as("primary_media");
  return db.select({ id: products.id, slug: products.slug, category: products.category, brand: products.brand, model: products.model, description: products.description, storage: products.storage, ram: products.ram, color: products.color, condition: products.condition, price: products.salePrice, warrantyDays: products.warrantyDays, quantity: sql<number>`coalesce((select sum(pb.available_quantity) from purchase_batches pb where pb.product_id=${products.id}),0)`, imageUrl:primaryMedia.imageUrl })
    .from(products).leftJoin(primaryMedia,eq(primaryMedia.productId,products.id)).where(and(eq(products.isPublished, true), category ? eq(products.category, category) : undefined, sql`coalesce((select sum(pb.available_quantity) from purchase_batches pb where pb.product_id=${products.id}),0)>0`)).orderBy(asc(products.brand), asc(products.model));
}

export async function getPublicProduct(slug:string){if(!databaseUrl())return undefined; const db=getDb(); const primaryMedia=db.select({productId:media.productId,imageUrl:sql<string|null>`(array_agg(${media.url} order by ${media.sortOrder},${media.id}))[1]`.as("image_url")}).from(media).groupBy(media.productId).as("primary_media"); const [product]=await db.select({id:products.id,slug:products.slug,category:products.category,brand:products.brand,model:products.model,description:products.description,storage:products.storage,ram:products.ram,color:products.color,condition:products.condition,price:products.salePrice,warrantyDays:products.warrantyDays,quantity:sql<number>`coalesce((select sum(pb.available_quantity) from purchase_batches pb where pb.product_id=${products.id}),0)`,imageUrl:primaryMedia.imageUrl}).from(products).leftJoin(primaryMedia,eq(primaryMedia.productId,products.id)).where(and(eq(products.slug,slug),eq(products.isPublished,true))).limit(1); return product; }
